...
const JS_OUTPUT_START = "'use strict'; angular.module('cf.ui').constant('icons',"
const JS_OUTPUT_END = ');'

const fs = require('fs')
const path = require('path')
const Promise = require('promise')
const svg2js = require('../node_modules/svgo/lib/svgo/svg2js')
const js2svg = require('../node_modules/svgo/lib/svgo/js2svg')
...
const jsOutputFile = process.argv[3]

console.log('Preparing SVG!\n')

const rawSVGIcons = filterSVGFiles(fs.readdirSync(inputDir))
...
const parsedSVGIcons = rawSVGIcons
...
function expandViewbox (icon) {
  const el = icon.svgTree.content[0]
  const viewBox = el.attrs.viewBox.value.split(' ').map(function (coord) {
