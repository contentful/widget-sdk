...
import { getPeriods, loadPeriodData, getAssetBandwidthData } from '../services/UsageService';
...
export const WorkbenchContent = () => {
  const { hasSpaces, isLoading, error } = useUsageState();
...
export const OrganizationUsageRoute = () => {
  ...
  useEffect(() => {
    ...
    const fetchOrgData = async () => {
      try {
        ...
        const data = await loadPeriodData(orgId, periods[selectedPeriodIndex]);
        const assetBandwidthData = await getAssetBandwidthData(orgId);
...
export const OrganizationUsageRouteNew = ({ orgId }) => {
  return (
...
OrganizationUsageRouteNew.propTypes = {
