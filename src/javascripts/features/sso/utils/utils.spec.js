...
describe('SSO utils', () => {
  describe('#validate', () => {
    it('should return empty string if no validator exists for given fieldName', () => {
      expect(utils.validate('aDifferentField', 'some value')).toBe('');
    });

    it('should returns empty string if valid value for fieldName', () => {
      expect(utils.validate('myAwesomeField', 'secret')).toBe('');
    });

    it('should returns error string if invalid valid value for fieldName', () => {
      expect(utils.validate('myAwesomeField', 'not secret')).toBe('Field is not valid');
    });
  });
  ...
  describe('#fieldErrorMessage', () => {
    ...
    it('should have custom errors for ssoName, idpCert, and idpSsoTargetUrl', () => {
      expect(utils.fieldErrorMessage('ssoName')).not.toBe(defaultErrorMessage);
      expect(utils.fieldErrorMessage('idpCert')).not.toBe(defaultErrorMessage);
      expect(utils.fieldErrorMessage('idpSsoTargetUrl')).not.toBe(defaultErrorMessage);
    });

    it('should have return the default error otherwise', () => {
      expect(utils.fieldErrorMessage('otherField')).toBe(defaultErrorMessage);
    });
  });
