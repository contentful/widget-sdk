...
export function SpacePurchaseRoute({ orgId }: Props) {
  ...
  const spaceId = navigationState?.spaceId;
  ...
  const from = navigationState?.from || queryParams.from;
  const preselect = navigationState?.preselect || queryParams.preselect;
