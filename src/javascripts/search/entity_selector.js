...
angular.module('contentful').factory('entitySelector', [
  ...
  require => {
    ...
    function prepareQueryExtension(config) {
      ...
      if (config.entityType === 'Asset') {
        ...
        if (Array.isArray(groups) && groups.length > 0) {
          extension['fields.file.contentType[in]'] = _.reduce(
            groups,
            (cts, group) => cts.concat(mimetype.getTypesForGroup(group)),
            []
          ).join(',');
        }
    ...
    function getLabels(config) {
      var key = [config.entityType.toLowerCase(), config.multiple ? 'multiple' : 'single'].join(
        '_'
      );
