...
export const getValidationsFormFields = (ctField) => {
  const fieldValidations = validationDecorator.decorateFieldValidations(ctField);
  return fieldValidations.reduce(
    (acc, validation) => ({
      ...
      [validation.type]: {
        ...
        validator: (value) => {
          const error = validationDecorator
            .validate(value)
...
export const getNodeValidationsFormFields = (ctField) => {
  ...
  const nodeValidations = validationDecorator
    .decorateNodeValidations(validation ? validation.nodes : {})
    .map((i) => i.validations)
    .flat();
