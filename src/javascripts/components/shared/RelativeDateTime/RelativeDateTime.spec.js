...
describe('RelativeDateTime renders the date string relative to now()', () => {
  ...
  it('has progressive timer that updates rendered date', () => {
    ...
    expectInTime(wrapper, 'a few seconds ago', moment().add(44, 's').valueOf());
    expectInTime(wrapper, 'a minute ago', moment().add(45, 's').valueOf());
    expectInTime(wrapper, '2 minutes ago', moment().add(1, 'm').valueOf());
    expectInTime(wrapper, '4 minutes ago', moment().add(2, 'm').valueOf());
    expectInTime(wrapper, 'an hour ago', moment().add(1, 'h').valueOf());
    expectInTime(wrapper, '3 hours ago', moment().add(2, 'h').valueOf());
  });
});
