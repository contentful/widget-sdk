    ...
    return {
      template: h('ul.search-autocomplete-list', [
        ...
        h(
          ...
          [
            h('span.item-value', [h('span.value-pill', ['{{item.value}}'])]),
      ...
      controller: [
        ...
        $scope => {
          ...
          function selectInitialAutocompletion() {
            ...
            $scope.selectedAutocompletion = _.find(
              $scope.autocompletion.items,
              i => i.value.toString() === token
            );
          }

          function selectNextAutocompletion() {
            const index = getSelectedIndex();
            $scope.selectedAutocompletion =
              $scope.autocompletion.items[index + 1] || $scope.autocompletion.items[0];
          }

          function selectPreviousAutocompletion() {
            const index = getSelectedIndex();
            $scope.selectedAutocompletion =
              $scope.autocompletion.items[index - 1] ||
              $scope.autocompletion.items[$scope.autocompletion.items.length - 1];
          }
          ...
          function getSelectedIndex() {
            return _.findIndex(
              $scope.autocompletion.items,
              i =>
                i.value === ($scope.selectedAutocompletion && $scope.selectedAutocompletion.value)
            );
          }
