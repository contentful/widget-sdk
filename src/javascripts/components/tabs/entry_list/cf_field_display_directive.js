  ...
  link: function (scope) {
    ...
    scope.dataForArray = (entry, field) => {
      ...
      if (hasItemsOfType(items, 'Asset')) {
        return _.map(filterVisibleItems(items), entry => scope.dataForAsset(entry, 'data.fields.file'));
      }
    };
    ...
    scope.displayLocation = value => value ? parseLocation(value.lat) + ', ' + parseLocation(value.lon) : '';
