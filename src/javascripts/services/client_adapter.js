...
.factory('clientAdapter', ['$injector', function ClientAdapter ($injector) {
  ...
  var createRequestQueue = $injector.get('clientAdapter/requestQueue');
  ...
  return {
    ...
    request: createRequestQueue(request)
  };

  function initialize (token) {
    baseUrl = '//' + environment.settings.api_host.replace(ENDING_SLASH, '');
    defaultHeaders = {
  ...
  function request (req) {
    if (!baseUrl || !defaultHeaders) {
      throw new Error('Call #initialize(token) first!');
    }
  ...
  function buildRequest (data) {
    var req = {
      method: data.method,
      url: [baseUrl, data.path.replace(LEADING_SLASH, '')].join('/'),
...
.factory('clientAdapter/requestQueue', ['$injector', function ($injector) {
