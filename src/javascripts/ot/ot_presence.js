...
*/
.controller('otDocPresenceController', ['$scope', 'require', function ($scope, require) {
  ...
  controller.focus = function (fieldId) {
    var now = new Date();
    if (fieldId === lastFieldId && now - lastFocus < FOCUS_THROTTLE) return;
    lastFieldId = fieldId;
    lastFocus = now;
