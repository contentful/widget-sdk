  ...
  render () {
    ...
    return (
      <div className="webhook-template-form">
        <h2 className="webhook-template-form__title">
          {template.title}
        </h2>
        {template.description && (
          <div className="webhook-template-form__description">
            {template.description}
          </div>
        )}
        {template.fields.map(field => {
          ...
          return <div key={field.name} className="webhook-template-form__field">
            {['text', 'password'].includes(field.type) && <TextField
              id={id}
              name={field.name}
            ...
            />}
            {field.type === 'content-type-selector' && <React.Fragment>
              ...
              {templateContentTypes.length > 0 && (
                ...
                >
                  ...
                  {templateContentTypes.map(ct => <option key={ct.id} value={ct.id}>{ct.name}</option>)}
                </select>
              )}
              {templateContentTypes.length < 1 && (
                <div className="webhook-template-form__error">
                  <ValidationMessage>
                    You need to have at least one content type with a valid title field to set up a webhook with this template.
                  </ValidationMessage>
                </div>
              )}
            </React.Fragment>}
