  ...
  describe('Recognises 12h and 24h formats', () => {
    it.each([
      ...
      (received, expectedDisplayValue, expectedValue, now) => {
        ...
        expect(renderResult.getByTestId('time').value).toBe(expectedDisplayValue);
      }
    );
    ...
    ])('falls back to value if input is invalid', (input, value, expectedDisplayValue) => {
    ...
    ])('sorts the suggestions on given input', async (value, firstLastSuggestions) => {
      const [renderResult] = build({ value });

      fireEvent.focus(renderResult.getByTestId('time'));

      const allSuggestionNodes = await renderResult.findAllByTestId('time-suggestion');
    ...
    ])('decreases the time by half an hour on arrow up', (value, expectedValue, now) => {
      mockDate(dateNowSpy, now);
    ...
    ])('increases the time by half an hour on arrow down', (value, expectedValue, now) => {
      mockDate(dateNowSpy, now);
      const [renderResult, props] = build({ value });
      fireEvent.focus(renderResult.getByTestId('time'));
      fireEvent.keyUp(renderResult.getByTestId('time'), {
