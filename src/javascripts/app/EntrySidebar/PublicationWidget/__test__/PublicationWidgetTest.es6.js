...
export const createCommands = () => mapValues(commandTemplates, createCommand);
...
export const DEFAULT_TEST_PROPS = {
  status: 'draft',
...
export const createPublicationWidgetTest = config => () => {
  const { component: TestComponent, componentDefaultProps, customTests } = config;

  function render(props) {
    const allProps = {
      ...(componentDefaultProps || DEFAULT_TEST_PROPS),
      ...props
    };
    const wrapper = renderReact(<TestComponent isSaving={false} {...allProps} />);
    return { wrapper, props: allProps };
  }

  afterEach(cleanup);

  customTests && customTests(render);
...
export function expectPrimaryButton(wrapper, { text, isDisabled, restriction = false }) {
