...
import { snapshotSidebarlist as template } from 'app/snapshots/SnapshotSidebarListTemplates.es6';
 ...
 */
registerDirective('cfSnapshotSidebarList', [
  'spaceContext',
  spaceContext => ({
    restrict: 'E',
    template,
    controller: [
      ...
      $scope => {
        ...
        const publishedVersion$ = otDoc.sysProperty
        ...
        // version changes.
        const snapshotLoad$ = publishedVersion$
          .flatMapLatest(() =>
            K.promiseProperty(
              spaceContext.cma
                .getEntrySnapshots(entryId, { limit: PREVIEW_COUNT })
                .then(res => res.items)
            )
          )
          .toProperty();
        ...
        // progress
        const isLoading$ = snapshotLoad$.map(load =>
          caseof(load, [[K.PromiseStatus.Pending, _.constant(true)], [null, _.constant(false)]])
        );
        ...
        const hasError$ = snapshotLoad$.map(load =>
          caseof(load, [[K.PromiseStatus.Rejected, _.constant(true)], [null, _.constant(false)]])
        );
        ...
        const snapshots$ = snapshotLoad$.scan(
          (prev, load) =>
            caseof(load, [
