 ...
 */
export function create(initialEntity, contentType, spaceEndpoint, saveThrottleMs = THROTTLE_TIME) {
  // A single source of Truth, properties like sys$ and data$ reflect the state of this variable.
  const entity = cloneDeep(initialEntity.data);
  normalize();
  ...
  return {
    ...
    async insertValueAt(path, i, value) {
      const current = getValueAt(path);
      // Silently fail on non-array field modification.
      if (current && !Array.isArray(current)) {
        return entity;
