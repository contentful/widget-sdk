  ...
  require => {
    ...
    class Relaunch extends React.Component {
      ...
      async componentDidMount() {
        ...
        this.unsubscribeFromOnboarding = $rootScope.$on(
          MODERN_STACK_ONBOARDING_COMPLETE_EVENT,
          () => this.forceUpdate()
        );
        ...
        if (showRelaunch) {
          return (
            <WithLink link="copy" trackingElementId="onboarding_relaunched">
              {move => (
                <div
                  className="modern-stack-onboarding--relaunch-wrapper nav-bar__link"
                  onClick={move}>
                  <Icon
                    name="relaunch-onboarding"
                    className="modern-stack-onboarding--relaunch-icon"
                  />
