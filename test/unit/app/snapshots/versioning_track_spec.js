...
describe('Versioning tracking', function () {
  ...
  beforeEach(function () {
    ...
    this.assertAnalyticsCall = (event, data) => {
      sinon.assert.calledOnce(this.analyticsTrack.withArgs('versioning:' + event));
      Object.keys(data).forEach((key) => {
        expect(this.getTrackingData()[key]).toEqual(data[key]);
      });
    };
  ...
  describe('#trackableConfirmator', function () {
    ...
    it('does not track if was not discarded', function () {
      ...
      this.track.trackableConfirmator(_.noop)();
      this.$apply();
      sinon.assert.notCalled(this.analyticsTrack);
    });
  });
