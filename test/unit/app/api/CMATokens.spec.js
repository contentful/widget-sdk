...
describe('app/api/CMATokens', function () {
  beforeEach(function () {
    ...
    $http.whenRoute('GET', Config.apiUrl('users/me/access_tokens'))
      .respond((_method, _url, _data, _headers, params) => this.listTokenHandler(params));
    this.listTokenHandler.returns([200, {
      total: 1,
      items: [
        {sys: {id: 'TOKEN-ID'}, name: 'TOKEN-NAME'}
      ]
    }]);
    this.revokeToken = sinon.stub().returns([200]);
    $http.whenRoute('PUT', Config.apiUrl('users/me/access_tokens/:token_id/revoked'))
