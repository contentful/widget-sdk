...
describe('WidgetParametersUtils', function () {
  describe('.filterDefinitions', function () {
    const definitions = [{id: 'format'}, {id: 'ampm'}];
    const widget = {id: 'datePicker', custom: false};

    it('filters out time mode parameter if is date only', function () {
    ...
    it('retains time mode parameter if format includes time', function () {
      const values = {format: 'time'};
      const filtered = Utils.filterDefinitions(definitions, values, widget);
      expect(filtered.length).toBe(2);
    });

    it('retains time mode parameter if is not builtin date picker', function () {
      const values = {format: 'dateonly'};
    ...
    it('does not touch widgets different than date picker', function () {
      const values = {format: 'dateonly'};
  ...
  describe('.filterValues', function () {
    ...
    it('removes unknown parameter values', function () {
      const values = {unknown: true};
      const filtered = Utils.filterValues(definitions, values);
      expect(filtered).toEqual({});
    });

    it('retains known parameter values', function () {
      const values = {foo: true, unknown: true};
      const filtered = Utils.filterValues(definitions, values);
      expect(filtered).toEqual({foo: true});
    });

    it('removes undefined parameter values', function () {
      const values = {foo: undefined};
      const filtered = Utils.filterValues(definitions, values);
      expect(filtered).toEqual({});
    });

    it('returns an empty object if no parameters are defined', function () {
  ...
  describe('.markMissingValues', function () {
    const definitions = [{id: 'x', required: true}, {id: 'y'}];

    it('marks not provided required values as missing', function () {
      const missing = Utils.markMissingValues(definitions, {});
      expect(missing).toEqual({x: true});
    });

    it('does not mark as missing if provided or not required', function () {
  ...
  describe('.unifyEnumOptions', function () {
    ...
    it('converts shorthand options to labelled options', function () {
      const unified = Utils.unifyEnumOptions(definitions);
      expect(unified[0].options).toEqual([{one: 'one'}, {two: 'two'}]);
    });

    it('does not touch labelled options or parameters that are not Enums', function () {
      const unified = Utils.unifyEnumOptions(definitions);
