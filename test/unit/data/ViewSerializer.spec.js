describe('ViewSerializer', () => {
  ...
  describe('#serialize() and #unserialize() round-trip', () => {
    ...
    itRoundTrips('`displayFieldIds` array without touching it', {
    ...
    itRoundTrips(' `searchFilters` object', {
      searchFilters: [
        ['__status', '', 'draft'],
        ['fields.foo', 'lt', '0'],
        ['sys.id', '', '']
    ...
    itRoundTrips('whole view object', {
      id: 'VIEW_ID',
      order: {
        direction: 'descending',
        fieldId: 'updatedAt'
  ...
  describe('#unserialize()', () => {
    ...
    itUnflattens('removes `searchTerm` if `searchFilters` is present', {
    ...
    itUnflattens('removes `searchTerm` if `searchText` is present', {
      searchTerm: 'foo:bar text',
      searchText: 'text'
    }, {
      searchText: 'text',
