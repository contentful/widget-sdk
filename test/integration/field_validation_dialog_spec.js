...
xdescribe('validation dialog', () => {
  ...
  function clickSave() {
    dialog.domElement.find('button:contains(Save)').click();
  }
  ...
  function describeLengthValidation(validationPath) {
    ...
    it('can be enabled and set', () => {
      ...
      expect(getFieldProperty(scope, validationPath)).toEqual([{ size: { min: 10, max: 20 } }]);
    });
    ...
    it('changes custom error message', () => {
      ...
      const errorMessage = settings().find('input[aria-label="Custom error message"]');
  ...
  describe('range validation', () => {
    ...
    it('can be enabled and set', () => {
      ...
      expect(scope.field.validations).toEqual([{ range: { min: -0.1, max: 0.1 } }]);
    });
  ...
  describe('regexp validation', () => {
    ...
    it('can be enabled and set', () => {
      ...
      expect(scope.field.validations).toEqual([{ regexp: { pattern: 'foo|bar', flags: 'i' } }]);
    });
  ...
  describe('content type validation', () => {
    ...
    it('can select content types', () => {
      ...
      const ct1Checkbox = settings().find('label:contains("CT 1") input');
  ...
  describe('asset type validation', () => {
    ...
    it('shows previously selected type', () => {
      ...
      openDialog();
      const selected = settings().find('label:contains("Markup") input');
  ...
  describe('array length validation', () => {
    ...
    it('can be set', () => {
      ...
      expect(scope.field.validations).toEqual([{ size: { min: 10, max: 20 } }]);
    });
  });
  ...
  describe('file size validation', () => {
    ...
    function settings(locator) {
      return dialog.domElement.find('[aria-label="Specify allowed file size"]').find(locator);
    }

    it('can be set', () => {
      settings('[aria-label="Enable validation"]').click();
      ...
      expect(scope.field.validations).toEqual([{ assetFileSize: { min: 2048, max: null } }]);
    });
    ...
    it('selects "at least" as initial view and changes to between', () => {
      ...
      expect(scope.field.validations).toEqual([{ assetFileSize: { min: 1, max: 2 } }]);
    });
  });
  ...
  describe('image dimension validations', () => {
    ...
    function settings(selector) {
      return dialog.domElement.find('[aria-label="Specify image dimensions"]').find(selector);
    }

    it('can set width minimum and exact height', () => {
      settings('[aria-label="Enable validation"]').click();

      settings('input[aria-label="Enable image width validation"]').click();

      settings('[aria-label="Minimum image width"]')
        .val('100')
        .trigger('input');

      settings('input[aria-label="Enable image height validation"]').click();
